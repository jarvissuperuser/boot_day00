...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/05/23 16:38:00 by tmugadza          #+#    #+#             */
/*   Updated: 2017/05/23 16:38:04 by tmugadza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Control::setColumns() {
	...
	try {
		...
		if (pos > 0 && pos < 9) {
			...
			std::cout << calColWidth(people[pos-1].nick_name);
			std::cout << calColWidth(people[pos-1].birthday_date);
			std::cout << calColWidth(people[pos-1].darkest_secret);
			std::cout << calColWidth(people[pos-1].email_address);
			std::cout << calColWidth(people[pos-1].favorite_meal);
			std::cout << calColWidth(people[pos-1].login);
			std::cout << calColWidth(people[pos-1].underwear_color);
			std::cout << "|\n";
		} else{
			std::cout << "And that was not an index in the list...\n";
		}
	} catch (...) {
